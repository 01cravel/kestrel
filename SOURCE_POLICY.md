...
| Historical investable universe | Official regulator and market-reference listing records joined to Kestrel’s append-only daily manifest | Retain included and excluded members, stable identities, exact cutoff and retrieval times, selection/model/policy versions, ETF holdings and complete delisting outcomes. Corrections append a new valid-time version; updates and deletes are forbidden | Prospective bitemporal ledger connected. A separate certified portfolio snapshot opens only when every symbol has a timely archived adjusted close, stable point-in-time identity, and raw split and dividend query proof. Historical folds remain locked until every member has a complete independent outcome path |
...
| Corporate actions and total return | Exchange/issuer notices plus DTCC, normalized by a validated point-in-time service | Splits, dividends, mergers, symbol changes and delistings must be applied before return or risk calculations | The no-cost archive now binds adjusted closes to raw split and dividend coverage retrieved before each decision cutoff and fails closed if any declared symbol or proof is missing. Merger and delisting outcomes still require complete independent evidence; premium reference data stays locked until its measured value exceeds its cost |
