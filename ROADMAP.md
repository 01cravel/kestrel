...
- ✅ Immutable bitemporal universe and outcome ledger: each post-close decision snapshot binds stable identities, included and excluded members, exact availability/retrieval cutoffs, evidence hashes, ETF look-through and model/policy versions. The daily worker now advances every frozen member from the retained no-cost archive. Survivors and ticker changes accumulate continuous same-currency adjusted paths; cash and stock mergers complete only with matching inactive-listing and authoritative SEC/issuer terms. Missing proceeds, disappearance, late or conflicting evidence, currency changes and adjustment mismatches fail closed. SQLite blocks updates/deletes and corrections append. The challenger still requires cryptographic reconstruction and five untouched annual windows.
