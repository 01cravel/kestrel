...
- 🚧 A conservative five-year equity DCF now keeps the original operating-cash-flow-minus-all-capex result beside a maintenance-versus-growth owner-cash sensitivity. The downside still treats all productive-asset spending as recurring. Base and strong may reclassify only part or all of capex above point-in-time PP&E depreciation, and only when a dated issuer filing explicitly describes capacity or growth investment. Depreciation is a cross-check, never silently assumed to equal maintenance capex. Missing disclosure, fewer than three positive observations, inadequate beta evidence or terminal value above 80% fails closed; every output is a range, never a target.
...
6. 🚧 The first maintenance-versus-growth investment model is connected with dated issuer evidence, a visible zero-to-upper-bound growth range and a separate fail-closed science gate. Add directly quantified maintenance disclosures where issuers provide them, then add net borrowing and balance-sheet adjustments before treating the levered owner-cash proxy as full FCFE.
