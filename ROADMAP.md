...
- ✅ The page is honestly labelled Candidate Portfolio. Candidate 1 remains immutable history; Candidate 2 corrects its overlapping AI exposure by capping the direct AI-capex theme at 12% and adding payments, healthcare, energy and consumer essentials. The separate deterministic selector can replace the eight direct companies only from an already frozen universe snapshot. It requires stable identity, verified membership, a positive cutoff price, at least six of eight declared company descriptors, a versioned economic-theme classification and no SEC conflict; it records snapshot, manifest, evidence, theme-policy and candidate hashes, refuses any theme above 12%, and never opens a promotion gate.
...
- ✅ Full searchable ETF company ledger. Every verified non-cash company row now shows its source ETF, issuer holdings date, ETF weight, total-portfolio weight and capital-adjusted value. Tiny Vanguard positions use exact issuer market values instead of rounded display percentages. Cross-fund names remain separate without stable identity, and missing or stale sources are labelled partial rather than assumed absent.
- ✅ Append-only ETF holdings-and-fee evidence boundary. Issuer files/prospectuses and SEC Form N-PORT records now retain permanent fund/share-class identity, report and exact source clocks, reporting lag, original units/currency, cash, derivatives, reported totals, fees, record IDs/accessions, URLs and hashes. Corrections append, SQL blocks mutation, and live or incomplete holdings cannot certify a historical fold.
- 🚧 Point-in-time valuation reconstructs trailing P/E and free cash flow only from facts available by each filing date, pairs them with split-normalized prices and applies the matching mechanical adjustment to old per-share facts and share counts. Cash-flow yield uses operating cash flow less productive-asset purchases and compares each company only with its own history. Every working Yahoo close is independently checked against Nasdaq's official history with an exact-date, 1% tolerance rule; missing coverage or disagreement fails closed. The new international Candidate 2 names still need equivalent local-regulator and currency-complete valuation histories before this gate can pass.
...
- ✅ Immutable bitemporal universe and outcome ledger: each post-close decision snapshot binds stable identities, included and excluded members, exact availability/retrieval cutoffs, evidence hashes, ETF look-through and model/policy versions. The daily worker advances every frozen member and now ingests explicit merger terms from SEC filing packages or allow-listed issuer evidence. It retains effective date, exact cash/ISO currency and stock ratio/successor identity, source clocks, URLs, accessions and raw hashes. Named amendments append and supersede only in later folds; unlinked conflicts, missing terms, late evidence, identity mismatches, ticker disappearance and item codes fail closed. The challenger still requires cryptographic reconstruction and five untouched annual windows.
...
2. 🚧 Strengthen prices and corporate actions without imposing a permanent fee. The local archive certifies a decision session only when every declared portfolio symbol has a timely adjusted close, stable identity and retained raw split/dividend proof. Automatic later outcomes now fail closed and complete mergers/delistings only from authoritative terms; broader exchange/DTCC coverage remains an upgrade. Add Databento pay-as-you-go official closes when the free account is ready. Reconsider premium reference data only after measured avoided errors or improved decisions exceed its annual cost.
...
4. 🚧 The archived ETF holdings-and-fee contract is connected and fails closed on stale/late reports, share-class mismatches, incomplete totals, unresolved cash/derivatives, currency conflicts or missing fees. Populate it prospectively from issuer files/prospectuses and SEC Form N-PORT; do not backfill current portfolios into old dates. Five untouched annual windows still have to accumulate before historical look-through can pass.
5. 🚧 As-filed SEC earnings, free-cash-flow valuation and the independent Nasdaq historical-price cross-check are connected. Extend equivalent point-in-time local-regulator, currency and price evidence to every Candidate 2 international company before this gate can pass.
...
- 🚧 The prediction journals still fall short of evidence for changing signals: the universe walk-forward ledger now captures delisting proceeds and successor paths, but the shorter-horizon journals do not yet retain full evidence/feature snapshots, evaluate Hold decisions and missed opportunities, or calculate uncertainty.
...
- 🚧 Record entry convention, benchmark, costs, corporate actions, delisting state and complete maximum drawdown. The universe ledger now retains automatic delisting state, proceeds and stock-successor paths; the shorter-horizon prediction journals still need that same terminal-state bridge.
...
- ✅ Build the bitemporal security master, point-in-time universe, archived ETF evidence and automatic outcome path. `universe_ledger.py` freezes append-only manifests and outcome versions; `etf_evidence.py` appends issuer/N-PORT holdings and fee reports; `universe_outcomes.py` follows stable identities through survivors, ticker changes and evidenced cash/stock mergers; `market_history.py` retains prices, listing vintages, source times and hashes. Five untouched annual windows must still mature before validation can pass.
