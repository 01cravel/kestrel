...
- ✅ Build the bitemporal security master, point-in-time universe and automatic outcome path. `universe_ledger.py` freezes append-only manifests and outcome versions; `universe_outcomes.py` follows stable identities through survivors, ticker changes and evidenced cash/stock mergers; `market_history.py` retains the prices, listing vintages, source times and hashes. Five untouched annual windows must still mature before validation can pass.
